...
export function getSharedNetworkDetails(): { type: string, ip: string } {
  ...
  if (typeof navigator !== 'undefined' && (navigator as any).connection) {
    ...
    return {
      type: `${connection.effectiveType || 'WiFi'} (H5 Browser)`,
      ip: '127.0.0.1 (Browser Restricted)'
    }
  }
  return {
    type: 'WiFi (H5 Default)',
    ip: '127.0.0.1'
  }
  // #endif
  ...
  try {
    const info = uni.getSystemInfoSync()
    return {
      type: `${String(info.platform || 'mp').toUpperCase()} (MiniProgram)`,
...
export async function browserShare(payload: SharePayload, platform: PluginPlatform): Promise<PluginResult | null> {
  // #ifdef H5
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (raw: any) => {
          resolve({
            ok: false,
            platform,
            message: `浏览器定位失败: ${String(raw?.message || 'unknown')}`,
            raw
          })
        },
        {
    ...
    uni.openLocation({
      ...
      fail: (raw: any) => {
        resolve({
          ok: false,
          platform,
          message: `打开地图失败: ${String(raw?.errMsg || 'unknown')}`,
          raw
        })
      }
    })
        ...
        uni.showActionSheet({
          ...
          success: (raw: any) => {
            resolve({
              ...
              platform,
              message: '已选择操作项',
              tapIndex: Number(raw?.tapIndex ?? -1),
              raw
            })
          },
          fail: (raw: any) => {
